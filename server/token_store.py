"""User settings persistence backed by Lakebase.

Stores per-user preferences (cluster ID, serving endpoint) in the
Lakebase PostgreSQL database. In local mode every function is a no-op.

PAT storage has been removed — deploy and run operations now use the
app service principal with ``run_as`` for user identity.
"""
