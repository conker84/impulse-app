...
_STEP_INSTRUCTIONS: dict[str, str] = {
    ...
    "channels": (
        ...
        "- Call `add_physical_signal` directly for each channel. Set `var_name` to a clean "
        "Python name (lowercase, underscores), `alias` and `channel_name` to the exact channel "
        "name from the Available Channels table. Add ALL the channels that match — do not stop "
        "after describing what you would add. **Act, don't narrate.**\n"
        "- After adding, briefly summarize what was added.\n\n"
